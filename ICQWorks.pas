...
const
  ...
  S_QIP_EVIL        = $00003000;
  S_QIP_DEPRESSION  = $00004000;
  S_QIP_ATHOME      = $00005000;
  S_QIP_ATWORK      = $00006000;
  S_QIP_LUNCH       = $00002001;
...
function GetTLVBlock(Pkt: PRawPkt): TTLVBlock;
...
procedure CreateCLI_IDENT(Pkt: PRawPkt; SN, Password: String; var Seq: Word);
...
procedure CreateCLI_LOCATE_USER_INFO_QUERY2(Pkt: PRawPkt; SN: string; LocateType: DWord; const RequestId: Word; var Seq: Word);
procedure CreateCLI_ICBM_CHANNEL_MSG_TOHOST(Pkt: PRawPkt; SN: string; const RequestId: Word; var Seq: Word);
procedure CreateCLI_BUCP_REQ_CHALLENGE(Pkt: PRawPkt; SN: string; var Seq: Word);
procedure CreateCLI_BUCP_QUERY(Pkt: PRawPkt; SN, Password, CKey: string; var Seq: Word);
...
function GetTLVBlock(Pkt: PRawPkt): TTLVBlock;
begin
  Result.T := GetInt(Pkt, 2);
  Result.L := GetInt(Pkt, 2);
  Inc(Pkt^.Len, Result.L);
end;
...
begin
  ...
  PktTLV(Pkt, 1, SN);                           //Adding user's UIN
...
begin
  ...
  PktInt(Pkt, $00010004, 4);                    //Family x01 is Version x03
  PktInt(Pkt, $00130003, 4);                    //Family x13 at Version x02
...
procedure CreateCLI_SETUSERINFO(Pkt: PRawPkt; var Seq: Word);
const
  caps: array[0..32 - 1] of Byte = (
    $09, $46, $13, $49, $4C, $7F, $11, $D1, $82, $22, $44, $45, $53, $54, $00, $00,
    $09, $46, $13, $4D, $4C, $7F, $11, $D1, $82, $22, $44, $45, $53, $54, $00, $00
  );
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
...
const
  buf: array[0..79] of Byte = (
    $00, $01, $00, $04, $01, $10, $04, $7B,
    $00, $13, $00, $03, $01, $10, $04, $7B,
...
begin
  PktInit(Pkt, 2, Seq);
  PktSnac(Pkt, $02, $15, (RequestId shl 16) or $15, 0);                
  PktInt(Pkt, LocateType, 4);
  PktLStr(Pkt, SN);
  PktFinal(Pkt);
end;
...
begin
  PktInit(Pkt, 2, Seq);
  PktSnac(Pkt, $04, $06, (RequestId shl 16) or $06, 0);                //  FG=ICBM, SNAC=0x0004;0x0006
  PktInt(Pkt, Random($51900000), 4);
  PktInt(Pkt, Random($00005190), 4);
  PktInt(Pkt, $02, 2);
  PktLStr(Pkt, SN);
  ...
  PktInt(Pkt, $000F0000, 4);                      //  ICBM_DEFS__RENDEZVOUS_TLV_TAGS_UUID_CHECK
...
procedure CreateCLI_BUCP_REQ_CHALLENGE(Pkt: PRawPkt; SN: string; var Seq: Word);
begin
  PktInit(Pkt, $02, Seq);                          //  Channel 2
  PktSnac(Pkt, $17, $06, 0, 0);                    //  FG=BUCP, SNAC=0x0017;0x0006
  PktTLV(Pkt, $0001, SN);
  PktFinal(Pkt);
end;
...
begin
  PktInit(Pkt, $02, Seq);                           //  Channel 2
  PktSnac(Pkt, $17, $02, 0, 0);                     //  FG=BUCP, SNAC=0x0017;0x0002
  PktTLV(Pkt, $0001, SN);
  PktTLV(Pkt, $0002, 16, oscarmd5__encode_pass(Password, CKey, 3));
  PktTLV(Pkt, $004C, 0, 0);                        
  PktFinal(Pkt);
end;
...
begin
  ...
  *)
  ...
  if (Value and S_DND) = S_DND then
    Result := S_DND
  else if (Value and S_QIP_EVIL) = S_QIP_EVIL then
    Result := S_QIP_EVIL
  else if (Value and S_QIP_DEPRESSION) = S_QIP_DEPRESSION then
    Result := S_QIP_DEPRESSION
  else if (Value and S_QIP_ATHOME) = S_QIP_ATHOME then
    Result := S_QIP_ATHOME
  else if (Value and S_QIP_ATWORK) = S_QIP_ATWORK then
    Result := S_QIP_ATWORK
  else if (Value and S_QIP_LUNCH) = S_QIP_LUNCH then
    Result := S_QIP_LUNCH
...
begin
  ...
  *)
  ...
  if (Value and S_DND) = S_DND then
    ...
    Result := 'Free For Chat'
  else if (Value and S_QIP_EVIL) = S_QIP_EVIL then
    Result := 'Online-Evil'
  else if (Value and S_QIP_DEPRESSION) = S_QIP_EVIL then
    Result := 'Online-Depression'
  else if (Value and S_QIP_ATHOME) = S_QIP_ATHOME then
    Result := 'Online-Home'
  else if (Value and S_QIP_ATWORK) = S_QIP_ATWORK then
    Result := 'Online-Work'
